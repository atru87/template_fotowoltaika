...
// WERSJA HYBRYDOWA - Redis w produkcji, pliki w dev mode
...
// W dev mode używamy fs do zapisu
let fs, path;
if (IS_DEV) {
  try {
    fs = require('fs');
    ...
    console.log('fs/path not available, file writes will be disabled');
  }
}
...
const staticData = {
  ...
  'bot-config': () => Promise.resolve({ apiKey: '', systemPrompt: 'Jesteś pomocnym asystentem firmy.' }),
...
/**
 * Zapisuje dane (do Redis lub do pliku w dev mode)
...
export async function writeJSON(filename, data) {
  ...
  try {
    ...
    if (IS_DEV && fs && path) {
      ...
      // Jeśli Redis też dostępny, zsynchronizuj
      if (USE_REDIS) {
        try {
          await redis.set(key, data);
          console.log(`✅ Synced to Redis: ${key}`);
        } catch (redisError) {
          console.log(`⚠️ File saved but Redis sync failed:`, redisError.message);
        }
      }
    ...
    // 2. Jeśli Redis jest dostępny - zapisz do Redis (produkcja)
    if (USE_REDIS) {
      ...
      // 3. Bez Redis i bez fs - nie możemy zapisać (read-only Vercel)
      console.error(`❌ Cannot save ${key} - Redis not configured and filesystem is read-only`);
      return false;
    }
